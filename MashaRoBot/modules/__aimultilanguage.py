...
from MashaRoBot import pbot as Nezuko
...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("KAELA ᴀɪ αℓяєα∂у αcтιναтє∂ ιη тнιѕ cнαт")
            return
        await lel.edit(
            f"KAELA ᴀɪ ѕυccєѕѕƒυℓℓу α∂∂є∂ ƒσя υѕєяѕ ιη тнє cнαт  {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("KAELA ᴀɪ ωαѕ ησт αcтιναтє∂ ιη тнιѕ cнαт")
            return
        await lel.edit(
            f"KAELA ᴀɪ ѕυccєѕѕƒυℓℓу ∂єαcтιναтє∂ ƒσя υѕєяѕ ιη тнє cнαт {message.chat.id}"
        )
...
__help__ = """
**Cʜᴀᴛʙᴏᴛ**
KAELA cαη ∂єтє¢т & яєρℓу υρтσ 200 ℓαηgυαgєѕ
